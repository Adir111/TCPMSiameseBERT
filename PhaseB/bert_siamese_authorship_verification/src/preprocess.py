...
class TextPreprocessor:
    ...
    def encode_tokenized_chunks(self, tokenized_chunks):
        """
        ...
        for chunk in tokenized_chunks:
            encoded_chunk = self.encode_single_chunk(chunk)
    ...
    def encode_single_chunk(self, token_chunk):
        ids = self.tokenizer.encode(" ".join(token_chunk)).ids[:self.max_length]
        # ids = self.tokenizer.convert_tokens_to_ids(token_chunk[:max_len])
        attention_mask = [1] * len(ids)
        padding_length = self.max_length - len(ids)

        if padding_length > 0:
            ids += [self.tokenizer.token_to_id("[PAD]")] * padding_length
