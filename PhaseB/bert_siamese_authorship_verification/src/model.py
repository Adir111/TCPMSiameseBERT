...
class SiameseBertModel:
    ...
    class CombinedInputBertLayer(tf.keras.layers.Layer):
        ...
        def call(self, inputs, training=None, mask=None):
            ...
            bert_output = self.bert_model(input_ids=input_ids, attention_mask=attention_mask, training=training)
            return bert_output.last_hidden_state  # or .pooler_output if you prefer
    ...
    def __create_base_model(self):
        ...
        # )[0]
        bert_output = self.CombinedInputBertLayer(self.bert_model, self.max_len)(combined_input, training=self.bert_model.trainable)
